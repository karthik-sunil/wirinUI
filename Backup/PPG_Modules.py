...
def roll_mean(sig, fs, rolling_index, len_frames):
    rolling_mean = [0]*(rolling_index-1) # Array that stores the rolling mean values
    
    greater = []    # Used to calculate rolling mean
    x_peaks = []    # X coordinates of peaks
    y_peaks = []    # Y coordinates of peaks
    base = 0
    for i in range(rolling_index, fs*(len_frames+2)):
        rolling_mean.append(sum(smooth_data[i-rolling_index:i])/(rolling_index))
...
def shift_data_left(ppg_val, fs):
...
def new_values(ppg_val, fs, new_data):
    ppg_val[-fs:] = new_data
    return(ppg_val)
#ppg_val is buffer generated by working74    
def store_previous_rolling_mean(smooth_data, fs, rolling_index):
    prev_roll_mean = smooth_data[(j+1)*fs-rolling_index:(j+1)*fs]
    return(prev_roll_mean)
def master_ppg(ppg_val, sig, N, Wn): 
    smooth_data = filter_signal(sig, N, Wn)
    rolling_mean, x_peaks, y_peaks = roll_mean(sig, fs, rolling_index, len_frames)
    heart_rate = calc_hr(x_peaks,len_frames)
...
def store_previous_rolling_mean(smooth_data, fs, num_roll):
    ...
    return(prev_roll_mean)
>>>>>>> 6d0ae1526ea5f0d4a0d869082e7dd8368e69e263:Backup/PPG_Modules.py
